...
class App(customtkinter.CTk):
    ...
    CENTER_LAT, CENTER_LON = 21.0329787, 105.8222534
    ...
    def _setup_ui(self):
        ...
        # Nút tìm đường gần điểm
        self.edge_check_button = customtkinter.CTkButton(
            self.panel,
            text="Tìm các cạnh gần điểm",
            command=self.check_edges_near_click
        )
        self.edge_check_button.pack(pady=10)
        
        # Thêm widget hiển thị thông tin về đường đi
        self.info_frame = customtkinter.CTkFrame(self.panel)
        self.info_frame.pack(pady=80, fill="x", padx=padding)
    ...
    def check_edges_near_click(self):
        ...
        if not self.markers:
            ...
            return
        lat, lon = self.markers[-1].position
        self.status_label.configure(text="Đang tìm các cạnh gần điểm...", text_color="orange")

        def handle_result(edges):
            self.after(0, lambda: self.status_label.configure(
                text=f"Tìm thấy {len(edges)} cạnh gần điểm", 
                text_color="green"
            ))
            ...
            for u, v in edges:
                ...
                self.map_widget.set_path([u_coords, v_coords], color="blue", width=2)
